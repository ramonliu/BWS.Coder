...
export class WorkflowRunner extends ChatExecutor {
    ...
    private async runWorkflowStep(
        ...
        while (!isStepDone && turnCount < MAX_TURNS_PER_STEP && !globalCts?.token.isCancellationRequested) {
            ...
            let result: any;
            try {
                result = await this.executeAITurn(state, state.client, workflowMessages, images, globalCts, streamCts, step.providerId, state.client.getProviderName(), step.role, task);
