...
const locales: Record<SupportedLang, LocaleStrings> = {
    'zh-TW': {
        ...
        err_emptyContent: '❌ **格式錯誤**：`{0}` 操作的內容區塊不能為空。刪除檔案請改用 `delete`。\n\n**正確範例**：\n<tool_call>\n  <name>{0}</name>\n  <arguments>\n    <path>{1}</path>\n    <content>(具體代碼)</content>\n  </arguments>\n</tool_call>',
        err_replaceFormat: '❌ **格式錯誤**：`replace` 區塊必須包含完整的 `<search>` 與 `<replace>` 標籤。\n\n**正確範例**：\n<tool_call>\n  <name>replace</name>\n  <arguments>\n    <path>{0}</path>\n    <search>(待替換的原始段落)</search>\n    <replace>(替換後的目標段落)</replace>\n  </arguments>\n</tool_call>',
        err_executorStalled: '\n[系統提示] 指令逾時。',
        ...
        op_readTruncatedHint: '\n\n[提示：由於檔案較大（共 {0} 行），已自動讀取前 50 行內容。若要繼續讀取剩餘內容，請在 arguments 中使用 `<start_line>51</start_line>` 和 `<end_line>100</end_line>` 讀取特定行數。]',
        ...
        msg_groupPersonaRolePrompt: '\n\n[群聊指令] 你現在的角色是「{0}」。\n{1}\n\n請以這個身份，針對上文內容發表你的看法，或反駁其他人的觀點。\n⚠️ **注意：這是一場純文字對話辯論。請「直接輸出」你的發言內容，【絕對不要】使用 `<tool_call>` 標籤來建立檔案，也不需要生成總結報告。**',
    ...
    'zh-CN': {
        ...
        err_replaceFormat: '❌ **格式错误**：`replace` 区块必须包含完整的 `<search>` 与 `<replace>` 标签。\n\n**正确示例**：\n<tool_call>\n  <name>replace</name>\n  <arguments>\n    <path>{0}</path>\n    <search>(旧)</search>\n    <replace>(新)</replace>\n  </arguments>\n</tool_call>',
        err_executorStalled: '\n[系统提示] 指令超时。',
        ...
        op_readTruncatedHint: '\n\n[提示：由于文件较大（共 {0} 行），已自动读取前 50 行内容。若要继续读取剩余内容，请在 arguments 中使用 `<start_line>51</start_line>` 和 `<end_line>100</end_line>` 读取特定行数。]',
        ...
        msg_groupPersonaRolePrompt: '\n\n[群聊指令] 你现在的角色是“{0}”。\n{1}\n\n请以这个身份，针对上文内容发表你的看法，或反驳其他人的观点。\n⚠️ **注意：这是一场纯文字对话辩论。请“直接输出”你的发言内容，【绝对不要】使用 `<tool_call>` 等任何标签来创建文件，也不需要生成总结报告。**',
    ...
    'en': {
        ...
        err_replaceFormat: '❌ **Format Error**: The `replace` block must contain complete `<search>` and `<replace>` tags.\n\n**Correct Example**:\n<tool_call>\n  <name>replace</name>\n  <arguments>\n    <path>{0}</path>\n    <search>(Old block)</search>\n    <replace>(New block)</replace>\n  </arguments>\n</tool_call>',
        ...
        op_readTruncatedHint: '\n\n[Hint: File too large ({0} lines total). Read first 50 lines automatically. To read more, use `<start_line>` and `<end_line>` in tool arguments for specific ranges.]',
